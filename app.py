...
REFERENCE_LYRICS_DIR = os.path.join(BASE_DIR, "reference_lyrics")
DEFAULT_MELODY = os.path.join(OLLAMA_DIR, "generated_melody.wav")

# Create directories if they don't exist
for directory in [OLLAMA_DIR, CUSTOM_DIR, REFERENCE_LYRICS_DIR]:
...
def calculate_rhyme_consistency(lyrics):
    ...
    for quatrain in quatrains:
        ...
        if (last_words[0][-2:] == last_words[1][-2:] and last_words[2][-2:] == last_words[3][-2:]) or \
           (last_words[0][-2:] == last_words[2][-2:] and last_words[1][-2:] == last_words[3][-2:]) or \
           (last_words[0][-2:] == last_words[3][-2:] and last_words[1][-2:] == last_words[2][-2:]):
            consistent_quatrains += 1
    return (consistent_quatrains / total_quatrains * 100) if total_quatrains > 0 else 0

def calculate_theme_adherence(lyrics, theme):
    words = re.findall(r'\b\w+\b', lyrics.lower())
    theme_words = THEME_WORDS.get(theme, [])
    theme_word_count = sum(1 for word in words if word in theme_words)
    return (theme_word_count / len(words) * 100) if words else 0
...
def load_reference_lyrics(theme):
    filename = f"{theme}_reference_lyrics.txt"
    filepath = os.path.join(REFERENCE_LYRICS_DIR, filename)
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            return [f.read().strip()]
    except FileNotFoundError:
        st.error(f"Reference lyrics file '{filename}' not found in {REFERENCE_LYRICS_DIR}")
        return [""]
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Lyrics Comparison", "Model Architecture", "Training Metrics", "Lyrics Metrics"])
    ...
    with tab1:
        ...
        if generate_button:
            ...
            # Store lyrics in session state for metrics tab
            st.session_state['lyrics_ollama'] = lyrics_ollama
            st.session_state['lyrics_custom'] = lyrics_custom
    ...
    with tab2:
        ...
        - **Training Data**: Random Tomatoes dataset (500,000 samples)
    ...
    with tab4:
        st.header("Lyrics Metrics")
        st.markdown("Comparison of generated lyrics based on BLEU score, rhyme consistency, theme adherence, and simulated metrics.")
        
        if 'lyrics_ollama' in st.session_state and 'lyrics_custom' in st.session_state and 'theme' in st.session_state:
            theme = st.session_state['theme']
            lyrics_ollama = st.session_state['lyrics_ollama']
            lyrics_custom = st.session_state['lyrics_custom']
            
            if lyrics_ollama and lyrics_custom and lyrics_ollama != "Failed to generate lyrics." and lyrics_custom != "Failed to load or generate lyrics.":
                reference_lyrics_list = load_reference_lyrics(theme)
                
                # Calculate metrics for Custom Transformer
                custom_bleu = calculate_bleu(lyrics_custom, reference_lyrics_list)
                custom_rhyme = calculate_rhyme_consistency(lyrics_custom)
                ...
                custom_theme = calculate_theme_adherence(lyrics_custom, theme)

                # Calculate metrics for Ollama model
                ollama_bleu = calculate_bleu(lyrics_ollama, reference_lyrics_list)
                ollama_rhyme = calculate_rhyme_consistency(lyrics_ollama)
                ...
                ollama_theme = calculate_theme_adherence(lyrics_ollama, theme)

                # Display Metrics
                st.subheader(f"Metrics Comparison (Theme: {theme.capitalize()})")
                col1, col2 = st.columns(2)
                
                with col1:
                    st.markdown("**Custom Transformer Metrics**")
                    st.write(f"- **BLEU Score**: {custom_bleu:.2f}")
                    st.write(f"- **Rhyme Consistency**: {custom_rhyme:.1f}%")
                    st.write(f"- **Training Stability**: {custom_stability}")
                    st.write(f"- **Long-term Coherence**: {custom_coherence:.2f}")
                    st.write(f"- **Resource Usage**: {custom_resources}")
                    st.write(f"- **Generation Speed**: {custom_speed} seconds")
                    st.write(f"- **Theme Adherence**: {custom_theme:.1f}%")
                
                with col2:
                    st.markdown("**Ollama LLaMA 3.1 Metrics**")
                    st.write(f"- **BLEU Score**: {ollama_bleu:.2f}")
                    st.write(f"- **Rhyme Consistency**: {ollama_rhyme:.1f}%")
                    st.write(f"- **Training Stability**: {ollama_stability}")
                    st.write(f"- **Long-term Coherence**: {ollama_coherence:.2f}")
                    st.write(f"- **Resource Usage**: {ollama_resources}")
                    st.write(f"- **Generation Speed**: {ollama_speed} seconds")
                    st.write(f"- **Theme Adherence**: {ollama_theme:.1f}%")
            else:
                st.warning("Please generate lyrics first to view metrics.")
        else:
            st.warning("Please generate lyrics first to view metrics.")
